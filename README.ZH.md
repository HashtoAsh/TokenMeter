...
必须带 `--features custom-protocol`，否则前端 `dist` 不会被内嵌，运行时会白屏。详见 [docs/quick-start.ZH.md](docs/quick-start.ZH.md)。
...
- 字段与命令接口见 [docs/api-design.ZH.md](docs/api-design.ZH.md)。
...
- [docs/quick-start.ZH.md](docs/quick-start.ZH.md) — 环境、构建与打包、使用说明、排障
- [docs/architecture.ZH.md](docs/architecture.ZH.md) — 模块划分、数据流、窗口状态机、存储
- [docs/api-design.ZH.md](docs/api-design.ZH.md) — Tauri 命令与事件、config schema、HTTP 请求形态
- [docs/tech-stack.ZH.md](docs/tech-stack.ZH.md) — 真实依赖清单与关键工程配置
- [PROGRESS.ZH.md](PROGRESS.ZH.md) — 每轮改进的"问题 → 根因 → 修法 → 验证"
...
[MIT](LICENSE)（第三方组件见 [THIRD_PARTY_LICENSES.ZH.md](THIRD_PARTY_LICENSES.ZH.md)）
